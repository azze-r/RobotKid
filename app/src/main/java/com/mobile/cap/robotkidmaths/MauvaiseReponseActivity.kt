...
import com.mobile.cap.robotkidmaths.databinding.ActivityMauvaiseReponseBinding
import java.util.Locale

class MauvaiseReponseActivity : AppCompatActivity() {
    private lateinit var binding: ActivityMauvaiseReponseBinding
    lateinit var textToSpeech: TextToSpeech

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        binding = ActivityMauvaiseReponseBinding.inflate(layoutInflater)
        ...
        textToSpeech = TextToSpeech(this) {status ->
            if (status == TextToSpeech.SUCCESS){
                ...
                textToSpeech.speak("Mauvaise réponse, le résultat est $message", TextToSpeech.QUEUE_FLUSH , null, "unique_id")
                Thread.sleep(3000)
